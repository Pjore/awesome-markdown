 ...
 * Exactly one of `privateKey` (inline PEM) or `privateKeyPath` (file path) must be set.
 * `webhookSecret` is reserved for Milestone 2.
 */
const GithubAppSchema = z
  .object({
    appId: z.string().min(1, 'GITHUB_APP_ID must not be empty'),
  ...
  .refine(
    ...
    {
      message:
        'One of GITHUB_APP_PRIVATE_KEY (inline PEM) or GITHUB_APP_PRIVATE_KEY_PATH (file path) must be set.',
    },
  );
...
export const EngineConfigSchema = z
  .object({
     ...
     * Required when `remote.enabled === true`.
     * Omit entirely for local-only operation.
     */
    githubApp: GithubAppSchema.optional(),
  })
  .refine(
    (v) => !(v.remote.enabled && !v.githubApp),
    {
      message:
        'remote.enabled is true but GitHub App credentials are not configured. ' +
        'Set GITHUB_APP_ID, GITHUB_APP_INSTALLATION_ID, and one of ' +
